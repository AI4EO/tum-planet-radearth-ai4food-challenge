...
class PixelSetEncoder(nn.Module):
    def __init__(
        ...
        self.output_dim = (
            input_dim * len(pooling.split("_")) if len(self.mlp2_dim) == 0 else self.mlp2_dim[-1]
        )
        ...
        assert input_dim == mlp1[0], f"{input_dim} != {mlp1[0]}"
        assert inter_dim == mlp2[0], f"{inter_dim} != {mlp2[0]}"
        # Feature extraction
        layers = []
